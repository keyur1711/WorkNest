...
export default function SpaceCard({ space, onToggleFavorite, isFavorite, commuteMins }) {
  ...
  return (
    <Link 
      to={`/spaces/${spaceId}`} 
      className="group block bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 hover:border-blue-300"
    >
      {/* Image Section */}
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <div className="px-3 py-1.5 bg-white rounded-lg shadow-lg flex items-center gap-1">
            <svg className="w-4 h-4 text-yellow-500 fill-current" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
            <span className="text-sm font-bold text-gray-900">{space.rating || '4.5'}</span>
          </div>
        </div>
      ...
      <div className="p-6">
        ...
        {typeof commuteMins === 'number' && (
          <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 bg-green-50 border border-green-200 rounded-lg">
            <svg className="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
            <span className="text-xs font-semibold text-green-700">~{commuteMins} min away</span>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {space.amenities?.slice(0, 2).map((amenity, idx) => (
            <span 
              key={idx} 
              className="text-xs px-3 py-1 bg-gray-100 text-gray-700 font-medium rounded-lg"
            >
              {amenity}
            </span>
          ))}
          {space.amenities?.length > 2 && (
            <span className="text-xs px-3 py-1 bg-blue-50 text-blue-700 font-medium rounded-lg">
